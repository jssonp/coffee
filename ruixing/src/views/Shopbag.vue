<template>
    <div class="shopbag">
        <van-nav-bar title="购物袋" left-text="返回"  left-arrow 
        @click-left="back">
        <template #right>
        ...
        <div v-if="shopbagData.length >0">
            ...
            <div class="shopbag-box">
                <van-list
                    v-model="loading"
                    :finished="finished"
                    finished-text="别拉了，数据没了"
                    offset="100"
                    @load="loadShopbagData"
                    >
                    <van-swipe-cell class="shopbag-cell" :disabled="isEdit" 
                    v-for="(item,index) in shopbagData" :key="item.sid">
                        <div class="shopbag-item">
                            <div class="check">
                                <van-checkbox v-model="item.isChecked" @change="simpleSelect" icon-size="22px"></van-checkbox>
                            </div>
                            <div class="pro-info">
                                <div class="pro-img">
            ...
            button-color="#9C623A" @submit="commit">
                <van-checkbox v-model="checked" @click="allSelect">全选</van-checkbox>
            </van-submit-bar>

            <van-submit-bar v-show="isEdit" button-text="删除选择" 
            button-color="#9C623A" @submit="removeSelect">
                <van-checkbox v-model="checked" @click="allSelect">全选</van-checkbox>
            </van-submit-bar>
        </div>
        <div v-else>
            <van-empty description="购物车空了，去找你心仪的商品吧！">
            ...
            getShopbagData() {
                ...
                this.axios({
                    ...
                    if(res.data.code ==5000) {
                        ...
                        this.shopbagData =this.shopbagAllData.slice(this.start,this.start + this.count);
